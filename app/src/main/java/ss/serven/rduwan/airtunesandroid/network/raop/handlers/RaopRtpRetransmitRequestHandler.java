...
import ss.serven.rduwan.airtunesandroid.audio.AudioClock;
...
public class RaopRtpRetransmitRequestHandler extends SimpleChannelUpstreamHandler {
	...
	private class MissingPacket {
		 ...
		 * @param nextSecondsTime next possible play back time
		 */
		private void computeExpectedUntil(final double nextSecondsTime) {
			expectedUntilSecondsTime = 0.5 * nextSecondsTime + 0.5 * m_audioClock.convertFrameToSecondsTime(requiredUntilFrameTime);
		}
	}
	 ...
	 * @param nextSecondsTime next possible play back time
	 */
	private void markRetransmitted(final int sequence, final double nextSecondsTime) {
		...
		while (i.hasNext()) {
			...
			if (missingPacket.sequence == sequence) {
				s_logger.fine("Packet " + sequence + " arrived " + (missingPacket.expectedUntilSecondsTime - nextSecondsTime) + " seconds before it was due");
				i.remove();
			}
		}
	}
